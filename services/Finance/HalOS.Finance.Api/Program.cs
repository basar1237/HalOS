...
// Tenant + kullanıcı bağlamı JWT claim'lerinden çözülür (docs/04 §7, docs/07 §6). Finance aynı
...
// carisine ALACAK (net hakediş) + ödeme planı vade tarihi yazar; tenant TenantConsumeFilter ile
// mesajdan çözülür (BK-8). PaymentDue/PaymentMade/CollectionReceived event'leri el-yapımı outbox
// üzerinden yayınlanır; MassTransit outbox'ı kapalı.
builder.Services.AddHalOSMessaging<FinanceDbContext>(
    builder.Configuration,
    x => x.AddConsumer<SaleCompletedConsumer>());
...
builder.Services.AddAuthorization(options => options.AddFinancePolicies());

// --- OpenTelemetry izleme (docs/04 §8) ---
builder.Services
    .AddOpenTelemetry()
    .ConfigureResource(resource => resource.AddService("HalOS.Finance"))
    .WithTracing(tracing =>
    {
        tracing
...
HalOS.BuildingBlocks.Infrastructure.MigrationExtensions.ApplyMigrations<FinanceDbContext>(app.Services);
